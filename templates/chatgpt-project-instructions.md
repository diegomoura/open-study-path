...
17. During task publication, treat `study/roadmap.md` and `study/topics/` as immutable approved inputs. Do not require the owner to repeat this rule in the publication command.
18. At the start of task publication, read `instructions/42-integration-preflight.md`, derive the required providers from `study.config.yml` and verify each actual connection with a harmless read-only connector operation. A configured provider or available tool definition is not sufficient proof of authorization.
19. If any required connection is unavailable, create no external resources and do not partially publish. Name only the missing providers, tell the owner to connect them in this ChatGPT Project and provide: `Conectei <providers> ao ChatGPT. Verifique novamente e continue a publicação.` Never request API keys, tokens or passwords.
20. When the owner sends the standard connection message, re-run every required probe. If all pass, continue publication automatically under the original authorization, without another confirmation or an intermediate “connections verified” response.
