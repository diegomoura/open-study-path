...
1. Before changing the repository, read `AGENTS.md`, `.open-study-path/template.yml`, `.open-study-path/instance.yml` when it exists, and `instructions/manifest.yml`.
...
8. Instance setup, intake import, curriculum generation and task publication are separate operations. Do not combine them unless the owner explicitly requests it.
9. Use the intake provider configured in `study.config.yml`. Do not silently switch providers.
