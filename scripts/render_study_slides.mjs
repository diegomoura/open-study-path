...
function contentType(file) {
  return { ".html": "text/html; charset=utf-8", ".css": "text/css; charset=utf-8", ".svg": "image/svg+xml" }[path.extname(file).toLowerCase()] || "application/octet-stream";
}
...
async function renderDiagrams(root, sourceDir, buildDir) {
  ...
  for (const source of sources) {
    ...
    await execFileAsync(mmdc, ["-i", source, "-o", output, "-c", config, "-p", puppeteerConfig, "-b", "transparent"], {
      cwd: root, timeout: 120_000, maxBuffer: 8 * 1024 * 1024,
    });
    const value = await fs.readFile(output, "utf8");
...
async function renderTopic({ browser, root, origin, topic, check }) {
  ...
  const diagrams = await renderDiagrams(root, topicDir, buildDir);
  ...
  const meta = {
    ...
    renderer: { id: RENDERER_ID, puppeteer: PUPPETEER_VERSION, mermaid_cli: MERMAID_CLI_VERSION, pdf_lib: PDF_LIB_VERSION, strategy: "static_svg_isolated_page_merge" },
