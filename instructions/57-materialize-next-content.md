...
Every newly materialized lesson must pass `instructions/36-review-course-content.md` before merge. Materialization is authorship; course-content review is a separate pass that verifies the delivered lesson still teaches and assesses the approved contract.
...
9. create or refresh `state/content-reviews/TOPIC-000.yml` only after the independent review passes for the current content version;
...
After authoring and before marking the PR ready:
...
- their current content-review artifacts;
